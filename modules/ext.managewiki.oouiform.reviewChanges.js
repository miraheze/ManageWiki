			...
			$( '#managewiki-review' ).click( function () {
				...
				$( '#managewiki-form :input:not( #managewiki-submit-reason :input )' ).each( function () {
					if ( this.type == 'checkbox' && this.defaultChecked != undefined && this.defaultChecked != this.checked ) {
						dialog.content.$element.append( '<li>' + mw.message( 'managewiki-review-checkbox-changed', this.name.replace( 'wp', '' ).replace( '[]', '[' + this.value + ']' ), ( this.checked == true ? 'enabled' : 'disabled' ) ).parse() + '</li>' );
					} else if ( this.defaultValue != undefined && this.defaultValue != this.value ) {
						dialog.content.$element.append( '<li>' + mw.message( 'managewiki-review-input-changed', this.name.replace( 'wp', '' ), ( this.defaultValue ? this.defaultValue : mw.msg( 'managewiki-review-none' ) ), ( this.value ? this.value : mw.msg( 'managewiki-review-none' ) ) ).parse() + '</li>' );
					}
				} );

				if ( !dialog.content.$element.html() ) {
					dialog.content.$element.append( mw.message( 'managewiki-review-no-changes' ).parse() );
				}
