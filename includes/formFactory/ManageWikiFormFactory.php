			...
			foreach ( $wgManageWikiExtensions as $name => $ext ) {
				if ( !$ext['conflicts'] ) {
					$formDescriptor["ext-$name"] = [
						...
						'help' => ( isset( $ext['requires']['extensions'] ) && (bool)$ext['requires']['extensions'] ) ? "Requires: " . implode( ',', $ext['requires']['extensions'] ) . "." : null,
					...
					$formDescriptor["ext-$name"] = [
						...
						'default' => $wiki->hasExtension ( $name ),
						'disabled' => ( $ceMW ) ? !ManageWikiRequirements::process( $dbName, $ext['requires'], $context ) : 1,
						'help' => ( isset( $ext['requires']['extensions'] ) && (bool)$ext['requires']['extensions'] ) ? "Requires: " . implode( ',', $ext['requires']['extensions'] ) . "." . " Conflicts: {$ext['conflicts']}." : "Conflicts: {$ext['conflicts']}.",
						'section' => ( isset( $ext['section'] ) ) ? $ext['section'] : 'other',
					];
				}
			}
